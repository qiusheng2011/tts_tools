...
class TTSCli():
    ...
    def init_args(self):
        ...
        parse.add_argument("-i", "--inputfile", metavar="I",
                           type=str, help="文本文件")
        parse.add_argument("-t", "--tts-type", metavar="T",choices=["edge"], default="edge")
    ...
    def parse_args(self,*args, **kwargs):
        ...
        if self.args.tts_type == "edge":
            ...
            if self.args.voice_type:
                self.voice_type = self.voice_types.get(self.args.voice_type,None)
        self.list = self.args.list


    def execute(self, debug=False):
        if self.list:
            print("\n".join([ f"{k}\t{o.name_md5}" for k, o in self.voice_types.items()]))
